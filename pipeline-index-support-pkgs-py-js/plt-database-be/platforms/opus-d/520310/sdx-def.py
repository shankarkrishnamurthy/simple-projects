...
def svm_vcpu_affinity(arg):
    bm = dom.getElementsByTagName('PRIMARY_RESERVED_THREADS')[0].firstChild.data.replace('"','')
    return [i for i,v in enumerate(bm[::-1]) if v == '1'][1]
...
# Returns the matching interface name or empty string if not found.
def interfacename(arg):
    # There should be one argument which is the eth device name (e.g. eth3).
    if not arg:
        return ""
    eth=arg[0]
    for elem in dom.getElementsByTagName('INTERFACE'):
        # The tagname has double quotes around it so remove them.
        if re.sub('"', '', elem.childNodes[0].data) == eth:
            return elem.getAttribute("name")
    return "NONE"

# Returns the matching interface type or empty string if not found.
def interfacetype(arg):
    if not arg:
        return ""
    eth=arg[0]
    for elem in dom.getElementsByTagName('INTERFACE'):
    ...
    try:
        ...
        action = \
            {'svmaffinity': (svm_vcpu_affinity, None),
			 ...
			 'interfacename': (interfacename, param),  

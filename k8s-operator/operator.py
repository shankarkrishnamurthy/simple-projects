...
def getPodLabels(configmap):
    url = "{}/apis/shankar.com/v1/namespaces/{}/configmonitors".format(
        base_url, namespace)
    log.info("Calling getPodLabels for " + configmap + " Will compare")
    log.info("Query " + url)
    r = requests.get(url)
    # Issue the HTTP request to the appropriate endpoint
    response = r.json()
    # Extract the podSelector part from each object in the response
    pod_labels_json = [i['spec']['podSelector'] for i in response['items'] if i['spec']['configmap'] == configmap]
    result = [list(l.keys())[0] + "=" + l[list(l.keys())[0]] for l in pod_labels_json]
